...
const Payment = ({ status, ordenID, checkID }) => {
	...
	const paymentUpdate = async () => {
		...
		await Axios.put(
			`http://localhost:3001/orders/payment/${check[1]}?status=${status}`
		);
	};
	...
	const sendMail = async () => {
		const { data } = await Axios.post(
			`http://localhost:3001/orders/mail?status=${status}`
		);
	};
	...
	const confirmar = async () => {
		await Axios.put(
			`http://localhost:3001/orders/payment/${checkID}?status=Pagado`,
			pago
		);
